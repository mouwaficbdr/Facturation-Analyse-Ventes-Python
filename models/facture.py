...
class FactureManager:
    ...
    def ajouter_facture(self, numero_facture, client_ifu, remise, client, produits):
        """
...
#Fonctions pour donner la somme total des prosuits (mht, mttc, etc)       
def totaliser_facture(lignes_facture):
    """
    Prend une liste de lignes (dictionnaires) d'une même facture
    et retourne les totaux HT, TTC, etc.
    """
    total_ht = sum(float(l['total_ht']) for l in lignes_facture)
    total_ttc = sum(float(l['total_ttc']) for l in lignes_facture)
    total_tva = sum(float(l['tva']) for l in lignes_facture)
    # On peut aussi additionner d'autres champs si besoin
    return {
